...
private:
	...
	vector<shared_ptr<Appointment>> appointments; //все записи на услуги
public:
	Client() {}
...
class Authentication {
	...
	vector<shared_ptr<Admin>> admins;
	vector<shared_ptr<SuperAdmin>> superAdmins;
	vector<shared_ptr<Admin>> admins_to_approve;

public:
	bool authenticateClient(const string& login, const string& password, shared_ptr<Client>& currentClient);
	bool authenticateAdmin(const string& login, const string& password, shared_ptr<Admin>& currentAdmin);
	bool authenticateSuperAdmin(const string& login, const string& password, shared_ptr<SuperAdmin>& currentSuperAdmin);

	void registerClient(const string& login, const string& password, const string& name, const string& surname);
	void registerAdmin(const string& login, const string& password, int adminID);
